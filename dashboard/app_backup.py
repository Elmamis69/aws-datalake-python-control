...
def get_sqs_messages_for_dashboard(queue_url, max_messages=10):
...
def main():
    ...
    with tab2:
        ...
        if 'error' in sqs_data:
            ...
            queue_attrs = sqs_data['queue_attributes']
                    ...
                    for i, msg in enumerate(messages, 1):
                        with st.expander(f"📨 Mensaje {i} - ID: {msg['message_id'][:8]}...", expanded=i <= 3):
                            ...
                            if st.checkbox(f"Ver JSON completo - Mensaje {i}", key=f"json_{i}"):
                                st.json(msg['body'] if isinstance(msg['body'], dict) else msg['raw_body'])
